...
//	man  kaodata 破解
	__code_kaodata=(arr,row,col)=>{tmp={};tmp[0]=arr;tmp[1]=tmp[0].map(x=>Number(x).toString(2).padStart(row,0)).join('');tmp[2]=[];[...Array(tmp[1].length/24).keys()].map(x=>tmp[2].push(tmp[1].slice(24*x,24*x+24)));tmp[3]=[];tmp[2].map(x=>{[...Array(row).keys()].map(y=>{tmp[4]='';[...Array(col).keys()].map(z=>tmp[4]+=x[y+row*z]);tmp[4]=parseInt(tmp[4],2);tmp[3].push(tmp[4]);});});tmp=tmp[3];return tmp;};
